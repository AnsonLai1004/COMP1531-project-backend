import {
  ...
  reqDmCreate, reqDmMessages, reqSendMessageDm
} from './requests';
beforeEach(() => {
  requestClear();
});
...
describe('admin/user/remove/v1', () => {
  ...
  test('test - messages', () => {
    ...
    const channel = requestChannelsCreateV3(channelOwner.token, 'first', true);
    expect(reqChannelJoin(user.token, channel.channelId)).toStrictEqual({});
    expect(reqMessageSend(user.token, channel.channelId, 'Hello World!')).toStrictEqual({ messageId: 1 });
    expect(reqAdminUserRemove(globalOwner.token, user.authUserId)).toStrictEqual({});
    ...
    // dm
    const user2 = requestAuthRegister('validemail@gmail.com', '123abc!@#', 'Carson', 'abc');
    const dm = reqDmCreate(channelOwner.token, [user2.authUserId]);
    expect(reqSendMessageDm(user2.token, dm.dmId, 'Hello World!')).toStrictEqual({ messageId: 1 });

...
describe("Channels Functions Errors", () => {
    ...
    test('error channelsCreate', () => {  
        ...
        expect(channelsCreateV1(-999, "TheoAng", true)).toStrictEqual({error: "error"});
    })

    // ASSUMPTION - invalid authUserId returns error
    test('invalid ID channelsListV1 channelsListallV1', () => {  
        expect(channelsListV1(-999)).toStrictEqual({error: "error"});
        expect(channelsListallV1(-999)).toStrictEqual({error: "error"});
    })
})
